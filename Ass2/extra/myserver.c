...
void *server_action(void *arg) {
    ...
    switch(operation) {
		...
		case STATS:
			printf("server: STATS\n");
			if (strcmp(operand, "meetup") == 0) meetup_stats(operand);
			else if (strcmp(operand, "rw") == 0) rw_stats(operand);
			else strcpy(operand, "Invalid val parameter. Choose \"meetup\" or \"rw\"");
			sprintf(result_message, "server: STATS -- %s\n", operand);
			break;
       case DEBUG: 
            printf("server: DEBUG\n"); 
